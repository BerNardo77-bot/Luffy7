...
export default {
  ...
  run: async ({ msg, sock, args, command }) => {
    ...
    if (!chat.nsfw)
      ...
      let mediaList = []
      const url = `https://api.rule34.xxx/index.php?page=dapi&s=post&q=index&json=1&tags=${tag}&api_key=${encodeURIComponent(global.rule34?.apiKey || process.env.RULE34_API_KEY || "")}&user_id=${encodeURIComponent(global.rule34?.userId || process.env.RULE34_USER_ID || "")}`
